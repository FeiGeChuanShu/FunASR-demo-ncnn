...
int main(int argc, char** argv) {
    ...
    Vad vad_segmentor;
    if (vad_segmentor.init(std::string(argv[2]) + "/vad4", 0) < 0) {
        fprintf(stderr, "vad init failed\n");
        return -1;
    }
    fprintf(stderr, "1.vad init done\n");

    std::shared_ptr<SenseVoice> sense_voice(new SenseVoice());
    model_config_t asr_model_config;
    asr_model_config.encoder = std::string(argv[2]) + "/SenseVoice";
    asr_model_config.token = std::string(argv[2]) + "/SenseVoice.json";
